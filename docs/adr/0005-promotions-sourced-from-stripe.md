...
Only a successful pricing read makes the displayed price actionable. A failed or timed-out read shows pricing as unavailable and changes the checkout action into a retry, so a hardcoded fallback can never begin Checkout.
...
- An acknowledgement that a Promotion cannot apply survives a retry. A refreshed discounted display can then check out at the full price the visitor accepted. Binding the acknowledgement to a Promotion version and interval would close this.
