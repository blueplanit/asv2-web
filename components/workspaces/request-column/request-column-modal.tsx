...
export function RequestColumnModal({
    open,
    onClose,
    workspaceName,
    stripeAccountId,
    ...
    async function submit() {
        ...
        const overSize = files.find(
            (f) => f.file.size > COLUMN_REQUEST_MAX_PER_FILE_BYTES,
        );
        if (overSize) {
            setStatus({
                kind: "err",
                msg: "Each screenshot must be 5MB or smaller.",
            });
            return;
        }
        if (totalBytes > COLUMN_REQUEST_MAX_TOTAL_BYTES) {
            setStatus({
                kind: "err",
                msg: "Screenshots are too large (max 10MB total).",
            });
            return;
        }
        ...
        try {
            ...
            for (const item of files)
                fd.append("screenshots", item.file, item.file.name);

            const res = await fetch("/api/column-request", {
                method: "POST",
                body: fd,
            });
            if (!res.ok) {
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-sm text-slate-700">
                                ...
                                <div className="text-xs text-slate-500">
                                    JPEG / PNG / WEBP · Max{" "}
                                    {COLUMN_REQUEST_MAX_SCREENSHOTS} files, 10MB
                                    total
                                </div>
                            </div>
                            <button
