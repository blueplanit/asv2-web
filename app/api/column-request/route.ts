...
export async function POST(req: Request) {
  try {
    ...
    if (columnsText.length > COLUMN_REQUEST_MAX_TEXT_LENGTH) return new NextResponse(`Request text too long (max ${COLUMN_REQUEST_MAX_TEXT_LENGTH} characters)`, { status: 400 });
    ...
    if (sgStatus === 401 || sgStatus === 403) {
      clientMsg = "Email service authentication error. Please contact support.";
    } else if (sgStatus === 429) {
      status = 429;
      clientMsg = "Email service is temporarily rate-limited. Please try again in a few minutes.";
    } else if (sgStatus && sgStatus >= 500) {
      clientMsg = "Email service is temporarily unavailable. Please try again shortly.";
    } else if (e?.code === "ENOTFOUND" || e?.code === "ECONNREFUSED") {
      clientMsg = "Unable to reach email service. Please try again shortly.";
    }
